...
class MockInterface(object):
    ...
    def __init__(self, run_watcher=True):
        ...
        if run_watcher:
            self.ros_watcher = MockWatcher(self.topics_change_cb, self.services_change_cb, self.actions_change_cb)
    ...
    def reconfigure(self, config, level):
        print(config)
        logging.warn("""[{name}] Interface Reconfigure Request: \ntopics : {topics} \nservices : {services} \nactions : {actions}""".format(name=__name__, **config))
        try:
            ...
            logging.warn('[{name}] Ignored list {topics} containing malformed topic strings. Fix your input!'
                         .format(name=__name__, topics=str(config["topics"])))
        try:
            ...
            logging.warn('[{name}] Ignored list {services} containing malformed service strings. Fix your input!'
                         .format(name=__name__, services=str(config["services"])))
        try:
            ...
            logging.warn('[{name}] Ignored list {params} containing malformed param strings. Fix your input!'
                         .format(name=__name__, params=str(config["params"])))
        try:
            ...
            logging.warn('[{name}] Ignored list {actions} containing malformed action strings. Fix your input!'
                         .format(name=__name__, actions=str(config["actions"])))

        return config

    def add_service(self, service_name, ws_name=None, service_type=None):
        rospy.loginfo("[{name}] Adding service {service}".format(name=__name__, service=service_name))
        ...
        if service_type is None:
            try:
                ...
                if not service_type:
                    logging.warn('[{name}] Cannot Expose unknown service {service} (maybe it is a regex or doesn\'t exist yet?)'.format(name=__name__, service=service_name))
                    self.services_waiting.append(service_name)
                    return False
            except rosservice.ROSServiceIOException, e:
                logging.warn('[{name}] Error trying to Expose service {service} : {error}'
                             .format(name=__name__, service=service_name, error=e))
                self.services_waiting.append(service_name)
                return False
        ...
        self.services[service_name] = MockService(service_name, service_type)
        return True
    ...
    def del_service(self, service_name, force=False):
        if service_name in self.services:
            logging.info("[{name}] Deleting service {service}".format(name=__name__, service=service_name))
            if force:
    ...
    def expose_services(self, service_names):
        logging.info('[{name}] Exposing services : {services}'.format(name=__name__, services=service_names))
    ...
    def add_topic(self, topic_name, topic_type=None, allow_pub=True, allow_sub=True):
        ...
        if self.topics_args[topic_name] >= 0:
            rospy.loginfo("[{name}] Adding topic {topic}".format(name=__name__, topic=topic_name))

        if topic_type is None:
            try:
                topic_type, _, _ = rostopic.get_topic_type(resolved_topic_name)
                if not topic_type:
                    rospy.logwarn('[{name}] Cannot Expose unknown topic {topic} (maybe it is a regex or doesn\'t exist yet?)'.format(name=__name__, topic=topic_name))
                    self.topics_waiting.append(topic_name)
                    return False
            except rosservice.ROSServiceIOException, e:
                rospy.logwarn('[{name}] Error trying to Expose topic {topic} : {error}'.format(name=__name__, topic=topic_name, error=e))
                self.topics_waiting.append(topic_name)
                return False
        ...
        if self.topics_args[topic_name] == -2:
            self.topics[topic_name] = MockTopic(topic_name, topic_type, allow_pub=allow_pub, allow_sub=allow_sub)
    ...
    def del_topic(self, topic_name, noloss=False, force=False):
        ...
        if topic_name in self.topics:
            rospy.loginfo("[{name}] Deleting topic {topic}".format(name=__name__, topic=topic_name))
            if force:
    ...
    def add_param(self, param_name):
        logging.info("[{name}] Adding param {param}".format(name=__name__, param=param_name))
        ...
        if not rospy.has_param(param_name):
            logging.warn('[{name}] Cannot Expose unknown param {param} (maybe it is a regex or doesn\'t exist yet?)'
                          .format(name=__name__, param=param_name))
        ...
        self.params[param_name] = MockParam(param_name)
        return True
    ...
    def del_param(self, param_name, force=False):
        if param_name in self.params:
            logging.info("[{name}] Deleting param {param}".format(name=__name__, param=param_name))
            if force:
    ...
    def expose_params(self, param_names):
        logging.info('[{name}] Exposing params : {params}'.format(name=__name__, params=param_names))
        if not param_names:
            return
        ...
        for param_name in self.params_args:
            if not param_name[0] in param_names or not self.is_regex_match(param_name[0], param_names):
                logging.info("param %s not in param names %r" % (param_name[0], param_names))
    ...
    def add_action(self, action_name, action_type=None):
        logging.info("[{name}] Adding action {action}".format(name=__name__, action=action_name))
        ...
        if action_type is None:
            ...
            topic_type, _, _ = rostopic.get_topic_type(resolved_topic_name)
            if not topic_type:
                logging.warn('Cannot Expose unknown action %s (maybe it is a regex, or doesn\'t exist yet?)', action_name )
    ...
    def expose_actions(self, action_names):
        rospy.loginfo('[{name}] Exposing actions : {actions}'.format(name=__name__, actions=action_names))
        if not action_names:
            return
