...
from pyros.rosinterface import RosTopicIfPool
...
@nose.tools.nottest
class TestRosTopicIfPool(unittest.TestCase):
    """
    ...
    def setup_class(cls):
        # Start roslaunch
        TestRosTopicIfPool.launch = roslaunch.scriptapi.ROSLaunch()
        TestRosTopicIfPool.launch.start()
        ...
        TestRosTopicIfPool.empty_srv_process = TestRosTopicIfPool.launch.launch(empty_srv_node)
        TestRosTopicIfPool.trigger_srv_process = TestRosTopicIfPool.launch.launch(trigger_srv_node)

    @classmethod
    def teardown_class(cls):
        # ensuring all process are finished
        if TestRosTopicIfPool.empty_srv_process is not None:
            TestRosTopicIfPool.empty_srv_process.stop()
        if TestRosTopicIfPool.trigger_srv_process is not None:
            TestRosTopicIfPool.trigger_srv_process.stop()
...
# EXPOSE TOPICS + UPDATE Interface
    def test_topic_appear_expose_update(self):
        ...
        self.topic_if_pool.expose_topics([topicname])
        # every added topic should be in the list of args
        self.assertTrue(topicname in self.topic_if_pool.topics_args)
        # topic backend has not been created since the update didn't run yet
        self.assertTrue(topicname not in self.topic_if_pool.topics.keys())

        dt = DiffTuple([], [])
        # NOTE : We need to wait to make sure the tests nodes are started...
        with Timeout(5) as t:
            while not t.timed_out and topicname not in dt.added:
                topics, topic_types = self.get_system_state()
                dt = self.topic_if_pool.update(topics, topic_types)
        ...
        self.assertTrue(topicname in self.topic_if_pool.topics.keys())

        # cleaning up
        self.topic_if_pool.expose_topics([])
        # every added topic should be in the list of args
        self.assertTrue(topicname not in self.topic_if_pool.topics_args)
        # topic backend has not been created since the update didn't run yet
        self.assertTrue(topicname not in self.topic_if_pool.topics.keys())
    ...
    def test_topic_appear_update_expose(self):
        ...
        with Timeout(5) as t:
            while not t.timed_out and nonexistent_pub.resolved_name not in self.topic_if_pool.topics_available:
                topics, topic_types = self.get_system_state()
                dt = self.topic_if_pool.update(topics, topic_types)
        ...
        dt = self.topic_if_pool.expose_topics([topicname])
        # every exposed topic should remain in the list of args ( in case regex match another topic )
        self.assertTrue(topicname in self.topic_if_pool.topics_args)
        # make sure the topic backend has been created
        ...
        self.assertTrue(topicname not in self.topic_if_pool.topics_available)
    ...
    def test_topic_withhold_update_disappear(self):
        ...
        nonexistent_pub = rospy.Publisher(topicname, Empty, queue_size=1)
        with Timeout(5) as t:
            while not t.timed_out and topicname not in self.topic_if_pool.topics_available:
                topics, topic_types = self.get_system_state()
                dt = self.topic_if_pool.update(topics, topic_types)
        ...
        dt = self.topic_if_pool.expose_topics([])
        ...
        with Timeout(5) as t:
            while not t.timed_out and not topicname in self.topic_if_pool.topics_available:
                time.sleep(1)
...
@nose.tools.istest
class TestRosInterface1NoCache(TestRosTopicIfPool):
    ...
    def setUp(self):
        ...
        self.topic_if_pool = RosTopicIfPool()

    def tearDown(self):
        self.topic_if_pool = None


# Testing with Connection Cache
@nose.tools.istest
class TestRosInterfaceCache(TestRosTopicIfPool):
...
if __name__ == '__main__':
    ...
    rostest_nose.rostest_or_nose_main('test_topic_if_pool_no_cache', 'test_all', TestRosInterface1NoCache)

    # Note : Tests should be able to run with nosetests, or rostest ( which will launch nosetest here )
    rostest_nose.rostest_or_nose_main('test_topic_if_pool_cache', 'test_all', TestRosInterfaceCache)
